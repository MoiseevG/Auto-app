...
export default function ServicesPage() {
  ...
  useEffect(() => {
    ...
    // load all masters for assignment (visible to operators)
    if (user && user.role === 'operator') {
      fetch(`${API_BASE_URL}/users/masters`).then(r => r.ok ? r.json() : []).then(setAllMasters).catch(() => setAllMasters([]));
    }
  }, []);
